...
class NamedArgumentChecker extends ScalariformChecker {
  ...
  private def localVisit(checkString: Boolean, ignoreMethod: Regex)(ast: Any): List[ScalastyleError] =
    ast match {
      ...
      case Argument(Expr(List(GeneralTokens(List(Token(tokenType, _, offset, _)))))) =>
        val namelessLiteral = tokenType match {
          case Tokens.NULL | Tokens.FALSE | Tokens.TRUE | Tokens.INTEGER_LITERAL | Tokens.CHARACTER_LITERAL |
